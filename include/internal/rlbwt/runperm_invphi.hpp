...
template<typename LFType>
std::tuple<IntVec, IntVec, IntVec, size_t, size_t> rlbwt_to_invphi_lengths_and_samples(const std::vector<uchar>& bwt_heads, const std::vector<ulint>& bwt_run_lengths, LFType& lf, size_t* domain = nullptr, ulint* max_length = nullptr) {
    ...
    return {invphi_lengths, move_run_to_invphi, run_head_sa_samples, UNUSED_INTERVAL, UNUSED_SA};
}

template<typename LFType>
IntVec samples_to_invphi_interval_permutations(const IntVec& move_run_to_invphi, const IntVec& run_head_sa_samples, const size_t UNUSED_SA, const LFType& lf) {
    ...
    return invphi_interval_permutations;
}
...
std::tuple<IntVec, IntVec> rlbwt_to_invphi(const std::vector<uchar>& bwt_heads, const std::vector<ulint>& bwt_run_lengths, LFType& lf, size_t* domain = nullptr, ulint* max_length = nullptr) {
    assert(bwt_heads.size() == bwt_run_lengths.size());
    auto [invphi_lengths, move_run_to_invphi, run_head_sa_samples, _, UNUSED_SA] = rlbwt_to_invphi_lengths_and_samples(bwt_heads, bwt_run_lengths, lf, domain, max_length);
    auto invphi_interval_permutations = samples_to_invphi_interval_permutations(move_run_to_invphi, run_head_sa_samples, UNUSED_SA, lf);
    return {invphi_lengths, invphi_interval_permutations};
}
